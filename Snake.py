...
class Food(arcade.Sprite):
    ...
    def __init__(self, scale):
        ...
        if self.type == APPLE:
            super().__init__("apple.png", scale)
        elif self.type == PEAR:
            super().__init__("pear.png", scale)
        elif self.type == SHIT:
            super().__init__("shit.png", scale / 2)

        self.center_x = randrange(10, SCREEN_WIDTH - RECT_WIDTH)
        self.center_y = randrange(10, SCREEN_HEIGHT - RECT_HEIGHT)
...
class MyGame(arcade.Window):
    ...
    def setup(self):
        ...
        self.player_sprite = Snake("snake.png", SPRITE_SCALING)
        self.player_sprite.center_x = 30
        self.player_sprite.center_y = 30
        self.player_list.append(self.player_sprite)

        for i in range(10):
            food_sprite = Food(SPRITE_SCALING / 10)
            food_sprite.center_x = randrange(30, SCREEN_WIDTH - 30)
            food_sprite.center_y = randrange(30, SCREEN_HEIGHT - 30)
    ...
    def on_update(self, delta_time):
        ...
        if not self.lost and not self.end:
            ...
            if self.player_list[0].direction == RIGHT:
                self.player_list[0].center_x += MOVEMENT_SPEED
            elif self.player_list[0].direction == LEFT:
                self.player_list[0].center_x += -MOVEMENT_SPEED
            elif self.player_list[0].direction == UP:
                self.player_list[0].center_y += MOVEMENT_SPEED
            elif self.player_list[0].direction == DOWN:
                self.player_list[0].center_y += -MOVEMENT_SPEED
            ...
            if colliding:
                ...
                for food in colliding:
                    print(food.type)
                    if food.type == APPLE:
                        tail_sprite = Snake('snake.png', SPRITE_SCALING)
